...
word_lenths=[len(word) for word in sentence.split(' ') if word != 'the']
print(word_lenths)
...
infile=myfile.read()
thelist= len([x for x in infile.split(' ') if x in ['the', 'The']])
...
extract=[x for x in phrase.split(' ') if x.isdigit()]
print(extract)
